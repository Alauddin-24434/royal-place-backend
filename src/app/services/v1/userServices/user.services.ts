...
import { IUpdateUserInput, IUser, IUserQuery } from "../../../interfaces/v1/userInterfaces/user.interfaces";
...
const updateUserById = async (id: string, updateData: IUpdateUserInput) => {
  ...
  if (!updatedUser) throw new AppError("User not found or has been deleted!", 404);
...
const requestRefreshToken = async (refreshToken: string) => {
  try {
    const decoded = jwt.verify(refreshToken, envVariable.JWT_REFRESH_TOKEN_SECRET as string) as JwtDecodedPayload;
    ...
    if (err instanceof jwt.TokenExpiredError) throw new AppError("Refresh token expired", 401);
    else if (err instanceof jwt.JsonWebTokenError) throw new AppError("Invalid refresh token", 401);
    throw err;
  }
};
